  ...
  class TodoistAppsApiSyncRestQuickBody
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Todoist::TodoistAppsApiSyncRestQuickBody` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !acceptable_attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Todoist::TodoistAppsApiSyncRestQuickBody`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      self.text = if attributes.key?(:text)
        attributes[:text]
      end
      ...
      self.auto_reminder = if attributes.key?(:auto_reminder)
        attributes[:auto_reminder]
      else
        false
      end
    ...
    def ==(other)
      ...
      self.class == other.class &&
        ...
        auto_reminder == other.auto_reminder &&
        meta == other.meta
    end
    ...
    def hash
      [text, note, reminder, auto_reminder, meta].hash
    end
