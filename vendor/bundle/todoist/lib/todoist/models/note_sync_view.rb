...
module Todoist
  ...
  class NoteSyncView
    ...
    attr_accessor :posted_uid

    attr_accessor :content

    attr_accessor :file_attachment

    attr_accessor :uids_to_notify
    ...
    def self.openapi_types
      {
        ...
        posted_uid: :String,
        content: :String,
        file_attachment: :"Hash<String, NoteSyncViewFileAttachmentValue>",
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Todoist::NoteSyncView` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !acceptable_attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Todoist::NoteSyncView`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      self.posted_uid = if attributes.key?(:posted_uid)
        attributes[:posted_uid]
      end
      ...
      if attributes.key?(:file_attachment)
        if (value = attributes[:file_attachment]).is_a?(Hash)
          self.file_attachment = value
        end
      else
        self.file_attachment = nil
      end

      if attributes.key?(:uids_to_notify)
        if (value = attributes[:uids_to_notify]).is_a?(Array)
          self.uids_to_notify = value
        end
      else
        self.uids_to_notify = nil
      end
      ...
      self.posted_at = if attributes.key?(:posted_at)
        attributes[:posted_at]
      end

      if attributes.key?(:reactions)
        if (value = attributes[:reactions]).is_a?(Hash)
    ...
    def ==(other)
      ...
      self.class == other.class &&
        id == other.id &&
        posted_uid == other.posted_uid &&
        content == other.content &&
        file_attachment == other.file_attachment &&
        uids_to_notify == other.uids_to_notify &&
    ...
    def hash
      [id, posted_uid, content, file_attachment, uids_to_notify, is_deleted, posted_at, reactions].hash
    end
