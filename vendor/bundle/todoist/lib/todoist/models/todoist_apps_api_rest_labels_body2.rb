  ...
  class TodoistAppsApiRestLabelsBody2
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Todoist::TodoistAppsApiRestLabelsBody2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !acceptable_attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Todoist::TodoistAppsApiRestLabelsBody2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      self.new_name = if attributes.key?(:new_name)
        attributes[:new_name]
      end
    end
    ...
    def new_name=(new_name)
      if new_name.nil?
        fail ArgumentError, "new_name cannot be nil"
      end
