  ...
  class ExposedCollaboratorSyncView
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Todoist::ExposedCollaboratorSyncView` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !acceptable_attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Todoist::ExposedCollaboratorSyncView`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      self.email = if attributes.key?(:email)
        attributes[:email]
      end
      ...
      self.image_id = if attributes.key?(:image_id)
        attributes[:image_id]
      end
    ...
    def hash
      [id, full_name, email, timezone, image_id, is_deleted].hash
    end
